...
public class ArgumentParser {
	...
	public void parse(String[] args) {
		for (int i = 0; i < args.length; i++) {
			int indexOfNextElement = i + 1;
			if (isFlag(args[i])) {
				if (indexOfNextElement <= args.length - 1 && isValue(args[indexOfNextElement])) {
					map.put(args[i], args[indexOfNextElement]);
				} else {
					map.put(args[i], null);
				}
			}
	...
	public Path getPath(String flag) {
		if (flag == null || map.get(flag) == null) {
			return null;
		}
	...
	// TODO public int getPositiveInteger(String flag, int defaultValue)
